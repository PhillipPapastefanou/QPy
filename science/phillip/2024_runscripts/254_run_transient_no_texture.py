...
number_of_runs = 1024*10

n_soil_combs = 1

PARTITION = 'work'
RUN_DIRECTORY =  "/Net/Groups/BSI/scratch/ppapastefanou/simulations/QPy/2024_bench/254_run_transient_no_texture/"
...
namelist_base.jsb_forcing_ctl.transient_spinup_years.value = 300
...
gamma_stem_max_min = 125
gamma_stem_max_max = 250

# 2. Parameter kappa_stem kg m-3 MPa-1
gamma_stem_res_min = 10
gamma_stem_res_max = 40
...
vol_porosity_min = 0.4
vol_porosity_max = 0.6

vol_field_cap_min = 0.15
vol_field_cap_max = 0.4
...
hyd_cond_sat_min = 1.0e-8
hyd_cond_sat_max = 5.0e-5
...
pore_size_index_min = 0.10
pore_size_index_max = 0.50
...
# 14. Parameter silt 
gdd_t_air_req_min  = 425
gdd_t_air_req_max = 425
...
vol_porositys = rescale(slicer.get(), min = vol_porosity_min, max = vol_porosity_max)
vol_field_caps = rescale(slicer.get(), min = vol_field_cap_min, max = vol_field_cap_max)
vol_p_wilts = rescale(slicer.get(), min = vol_p_wilt_min, max = vol_p_wilt_max)
vol_wress = rescale(slicer.get(), min = vol_wres_min, max = vol_wres_max)
hyd_cond_sats_log = rescale(slicer.get(), min = np.log10(hyd_cond_sat_min), max = np.log10(hyd_cond_sat_max))
matric_pots = rescale(slicer.get(), min = matric_pot_min, max = matric_pot_max)
bclapps = rescale(slicer.get(), min = bclapp_min, max = bclapp_max)
pore_size_indexs = rescale(slicer.get(), min = pore_size_index_min, max = pore_size_index_max)
...
for j in range(0, n_soil_combs):
    ...
    for i in range(0, number_of_soil_samples):
        ...
        if i == 0:
            ...
            nlm.jsb_sse_nml.l_soil_texture.value = False
            nlm.jsb_hydro_nml.l_soil_texture.value = False
            
            # nlm.jsb_sse_nml.vol_porosity.value = float(vol_porositys[i])
            # nlm.jsb_sse_nml.vol_field_cap.value = float(vol_field_caps[i])
            # nlm.jsb_sse_nml.vol_p_wilt.value = float(vol_p_wilts[i])
            # nlm.jsb_sse_nml.vol_wres.value = float(vol_wress[i])
            # nlm.jsb_sse_nml.hyd_cond_sat.value = float(10**hyd_cond_sats_log[i])
            # nlm.jsb_sse_nml.matric_pot.value = float(matric_pots[i])
            # nlm.jsb_sse_nml.bclapp.value = float(bclapps[i])
            # nlm.jsb_sse_nml.pore_size_index.value = float(pore_size_indexs[i])
            
            
            # 1. Extract values to local variables for easy reading
            porosity = float(vol_porositys[i])
            field_cap = float(vol_field_caps[i])
            p_wilt = float(vol_p_wilts[i])
            wres = float(vol_wress[i])
            matric_pot = float(matric_pots[i])

            # Define a small physical buffer (1% volumetric water content)
            margin = 0.01 

            # 2. Enforce the hierarchy: Top-Down Correction
            # Check Field Capacity vs Porosity
            if field_cap >= porosity:
                field_cap = porosity - margin
                vol_field_caps[i] = field_cap  # Override array

            # Check Wilting Point vs Field Capacity
            if p_wilt >= field_cap:
                p_wilt = field_cap - margin
                vol_p_wilts[i] = p_wilt        # Override array

            # Check Residual Water vs Wilting Point
            if wres >= p_wilt:
                wres = max(0.001, p_wilt - margin) # Ensure wres doesn't drop below 0
                vol_wress[i] = wres            # Override array

            # 3. Safety check for matric potential (Must be strictly negative in JSBACH/QUINCY)
            if matric_pot >= 0.0:
                matric_pot = -0.001
                matric_pots[i] = matric_pot    # Override array

            # 4. Assign the checked and corrected values to the namelist
            nlm.jsb_sse_nml.vol_porosity.value = porosity
            nlm.jsb_sse_nml.vol_field_cap.value = field_cap
            nlm.jsb_sse_nml.vol_p_wilt.value = p_wilt
            nlm.jsb_sse_nml.vol_wres.value = wres

            # 5. Assign the remaining independent parameters 
            nlm.jsb_sse_nml.hyd_cond_sat.value = float(10**hyd_cond_sats_log[i])
            nlm.jsb_sse_nml.matric_pot.value = matric_pot
            nlm.jsb_sse_nml.bclapp.value = float(bclapps[i])
            nlm.jsb_sse_nml.pore_size_index.value = float(pore_size_indexs[i])
 ...
 # or pore_size_indexs[i] depending on how you named it
...
df_parameter_setup['vol_porosity'] = np.round(np.tile(vol_porositys, n_soil_combs), 5)
df_parameter_setup['vol_field_cap'] = np.round(np.tile(vol_field_caps, n_soil_combs), 5)
df_parameter_setup['vol_p_wilt'] = np.round(np.tile(vol_p_wilts, n_soil_combs), 5)
df_parameter_setup['vol_wres'] = np.round(np.tile(vol_wress, n_soil_combs), 5)

# Use 10** to store the actual physical value, and round to 10 decimals 
# because values like 1.0e-8 would become 0.0 if rounded to 5 decimals!
df_parameter_setup['hyd_cond_sat'] = np.round(np.tile(10**hyd_cond_sats_log, n_soil_combs), 10)

df_parameter_setup['matric_pot'] = np.round(np.tile(matric_pots, n_soil_combs), 5)
df_parameter_setup['bclapp'] = np.round(np.tile(bclapps, n_soil_combs), 5)
df_parameter_setup['pore_size_index'] = np.round(np.tile(pore_size_indexs, n_soil_combs), 5)
