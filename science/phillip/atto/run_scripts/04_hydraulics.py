...
RUN_DIRECTORY = "/Net/Groups/BSI/scratch/ppapastefanou/simulations/QPy/ATTO_example_04_hydraulics/"
...
# 1. Parameter clay fractions
clay_frac_min = 0.3
clay_frac_max = 0.6

# 2. Parameter sand fraction
sand_frac_min = 0.2
sand_frac_max = 0.4

# 3. Parameter growth respiration fraction (standard is 0.25)
fresp_growth_min = 0.15
fresp_growth_max = 0.35

# 4. Parameter stom closure 
psi_close50_min = -0.5
psi_close50_max = -1.5

# 5. Parameter k_xylem_sat
k_xylem_sat_min = 2
k_xylem_sat_max = 10

# 6. Parameter kappa_stem
kappa_stem_min = 50
kappa_stem_max = 400
...
# 8. Parameter kappa_leaf 
root_scale_min = 10.0
root_scale_max = 100.0


# Now we rescale parameters
sand_fracs = np.round(rescale(slicer.get(), min = sand_frac_min, max = sand_frac_max), 3)
clay_fracs = np.round(rescale(slicer.get(), min = clay_frac_min, max = clay_frac_max), 3)
fresps = np.round(rescale(slicer.get(), min = fresp_growth_min, max = fresp_growth_max), 3)
...
for i in range(0, number_of_runs):
    ...
    paramlist.vegetation_ctl.fresp_growth.value = fresps[i]
...
df_parameter_setup = pd.DataFrame(np.round(sand_fracs, 3))
df_parameter_setup.columns = ['sand_fracs']
df_parameter_setup['id'] = np.arange(0, number_of_runs)
df_parameter_setup['fid'] = np.arange(0, number_of_runs)
df_parameter_setup['clay_fracs'] = np.round(clay_fracs,3)
df_parameter_setup['fresp'] = np.round(fresps,3)
...
df_parameter_setup['k_xylem_sat']= np.round(k_xylem_sats,5)
...
shutil.copyfile(os.path.join(THIS_DIR, os.pardir, os.pardir, os.pardir, os.pardir,'src', 'quincy', 'run_scripts', 'run_mpi.py'), 
