...
for model in MODELS
    for ssp in ssps
        ...
        for var in vars
            ...
            @time df_old = get_multi_file_slice_avg(run_collections_old, var, Fluxnetdata, DailySeries, 0.1, 0.9, slice_dates, d1, d2 )
            @time df_new = get_multi_file_slice_avg(run_collections_new, var, Fluxnetdata, DailySeries, 0.1, 0.9, slice_dates, d1, d2 )
            ...
            save(joinpath(plt_dir,"$(model)_$(ssp)_$var.png"), fig)
        end
    end
end
