...
# Safe quantile function for observations (in case some hours only have 1 data point)
...
for scen in scenarios
    ...
    for variable in var_avails
        ...
        df = transform(df_ensemble, val_cols => ByRow((vals...) -> begin
            ...
            # UPDATED: Changed from 0.1/0.9 to 0.2/0.8 quantiles for model spread
        ...
        if obs_df_raw !== nothing
            ...
            # UPDATED: Calculate 0.2 and 0.8 bounds for observations over the time period
...
if haskey(all_data, var_x) && haskey(all_data, var_y)
    ...
    if haskey(all_data[var_x], "Obs") && haskey(all_data[var_y], "Obs")
        ...
        if nrow(df_h_obs) > 0
            ...
            # UPDATED: Plot the 2D variability using xerror and yerror (diffs from mean)
            ...
            scatter!(h_plot, df_h_obs.mean_x, df_h_obs.mean_y, 
                     xerror=xerr_obs, yerror=yerr_obs, color=:black, alpha=0.5, label="", markersize=6)
        else
    ...
    for (lab, col) in [(L"\psi_{s} + \psi_{L} + J", :red)]
        ...
        if nrow(df_h_mod) > 0
            ...
            # UPDATED: Plot the 2D variability (0.2 and 0.8 quantiles) as crosshairs
            ...
            scatter!(h_plot, df_h_mod.mean_x, df_h_mod.mean_y, 
                     xerror=xerr_mod, yerror=yerr_mod, color=col, alpha=0.5, label="", markersize=2)
    ...
    savefig(h_plot, joinpath(post_process_dir, "hysteresis_area_$(var_x)_vs_$(var_y).png"))
end
