...
sys.path.append(os.path.join(THIS_DIR, os.pardir, os.pardir, os.pardir,  os.pardir,  os.pardir))
...
RUN_DIRECTORY =  "/Net/Groups/BSI/scratch/ppapastefanou/simulations/QPy/2023_bench/69_run_transient_3days_new_mort_new_phen_fix/"
...
k_xylem_sat_min = 0.9
k_xylem_sat_max = 1.15

# 2. Parameter kappa_stem kg m-3 MPa-1
gamma_stem_max_min = 100
...
psi_leaf_thres_min = -3.5
psi_leaf_thres_max = -4.5
...
psi_leaf_thress = rescale(slicer.get(), min = psi_leaf_thres_min, max = psi_leaf_thres_max)
gress = rescale(slicer.get(), min = gres_min, max = gres_max)
...
for j in range(0, n_soil_combs):
    ...
    for i in range(0, number_of_soil_samples):
        ...
        lctlib[pft].g_res = float(gress[i])
        ...
        lctlib[pft].psi_leaf_threshold = float(psi_leaf_thress[i])
...
df_parameter_setup['gres']=np.round(np.tile(gress, n_soil_combs),5)
...
df_parameter_setup['psi_leaf_thres']= np.round(np.tile(psi_leaf_thress, n_soil_combs) ,5)
...
shutil.copyfile(os.path.join(THIS_DIR, os.pardir, os.pardir, os.pardir,  os.pardir,  os.pardir,'src', 'quincy', 'run_scripts', 
                             'run_quincy_array.py'), 
