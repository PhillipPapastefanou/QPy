...
RUN_DIRECTORY = "output/07_transient_fluxnet_test"
...
# We define min and max in Degree C
gdd_t_air_threshold_min = 1.0
gdd_t_air_threshold_max = 5.0

# Define the number of steps we want to slice
nslice = 4
# Now we can use numpy to create and array
gdd_t_air_threshold_arr = np.linspace(gdd_t_air_threshold_min, gdd_t_air_threshold_max, num=nslice)
...
for i in range(0, nslice):
    ...
    paramlist.phenology_ctl.gdd_t_air_threshold.value = gdd_t_air_threshold_arr[i]
...
df_parameter_setup = pd.DataFrame(gdd_t_air_threshold_arr)
df_parameter_setup.columns = ['gdd_t_air_threshold']
