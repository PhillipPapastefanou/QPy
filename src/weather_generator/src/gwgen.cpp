...
//
//
// double roundoff(double dval, int limit) {
...
// */
//double cldf2rad(Gridcell& gridcell, double input, double lat, int doy, bool cldf2rad) {
//
...
//    const double YEARLENGTHD = gridcell.year_length;
//
...
//            cldfr = std::max(0., std::min(1.,cldfr));
//        }
//        else {
//            cldfr = 0.;
//        }
...
//    wind = std::max(0.0, metvars.resid[3] * sqrt(std::max(0.0, metvars.dmwind_sd)) + sqrt(std::max(0.0, metvars.dmwind_mn)));
...
//                         (pow(std::max(wind_bias_min, std::min(wind_bias_max, metvars.resid[3])), i));
//        }
//    }
//
//    intercept_corr = 0.;
...
//                             wind_intercept_bias_a * std::max(wind_bias_min, std::min(wind_bias_max, metvars.resid[3])));
//    }
//    else {
//        for (i=0; i<6; i++) {
...
//    }
//
//    wind = (wind - intercept_corr) / std::max(slopecorr, 9e-4);
//
//    // tmin bias correction
//    for (int i=0; i<6; i++) {
//        tmin_bias += tmin_bias_coeffs[i] *
//                     (pow(std::max(tmin_bias_min, std::min(tmin_bias_max, metvars.resid[0])),i));
//    }
...
//
//    if ( limit[0] != 0. ) {
//        std::cout << "Error in weathergen.cpp redist_restricted_vals()\n";
//        exit(99);
//    }
//
//    bool go = true;
...
//        std::cout<< "Redistribution in weathergen.cpp failed!" ;
//        exit(99);
//    }
//}
//
...
//    double relhum = std::min(1., std::max(0., exp(rh1 - rh2)));
//    return relhum;
//}
//
...
//
//                        std::vector<double>&  out_dtemp, std::vector<double>&  out_dprec,
//                        std::vector<double>&  out_dsol, std::vector<double>&  out_ddtr,
//                        std::vector<double>&  out_dwind, std::vector<double>& out_drhum) {
//
//
//    WeatherGenState& rndst = gridcell.state;
//
//    double lat = gridcell.lat;
//    double lon = gridcell.lon;
//
...
//        int ndaymon = gridcell.ndaysmonth[mon];
//        //
...
//        int lm = std::max(mon-1,0);
//        int rm = std::min(11,mon+1);
//
//        // Index for rmsmooth
//        int ilm = 0;
//        int irm = 2;
...
//        double tmvals[3];
//        int cmdays[3] =
//                {gridcell.ndaysmonth[lm],gridcell.ndaysmonth[mon],gridcell.ndaysmonth[rm]};
//
//        // Smooth tmin
//        bcond[0]  = in_mtmin[lm] ;
//        bcond[1]  = in_mtmin[rm] ;
...
//        for (int day=0;day<ndaymon;day++) {
//            mcloud_curr[day] = std::max(0.01,mcloud_curr[day]);
//        }
//
//        // Smooth wind
//        bcond[0]  = in_mwind[lm];
...
//        for (int day=0;day<ndaymon;day++) {
//            mwind_curr[day] = std::max(0.1,mwind_curr[day]);
//        }
//
...
//        double prec_t = std::max(0.5,0.05 * in_mprec[mon]);
//
...
//        // but no wet days.
//        if ( metvars.mprec > 0. ) {
//            metvars.mwetd = std::max(1.,in_mwetd[mon]);
//        } else {
//            metvars.mwetd = 0.;
//        }
...
//            cldwght[day] = std::max(0.01,cldf2rad(gridcell,0.0,lat,doy,true));
...
//            for (int day=0;day<ndaymon;day++) {
//                doy = accumday+day+1;
//                dsol[day] = std::max(0.001,cldf2rad(gridcell, dcldf[day],lat,doy,true));
//                solcor += dsol[day];
//            }
